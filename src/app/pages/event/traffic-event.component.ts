...
export class TrafficEventComponent extends BaseErrorHandlerComponent implements OnInit {
  ...
  trackingTarget = ''; // License plate number
  trackingLocations: TrackingLocation[] = [];
  trackingLoading = false;
  selectedEventId = ''; // Selected event ID for map tooltip
  ...
  handleSearch(searchParams: any) {
    ...
    // Check if this is a tracking search (searching for a specific license plate)
    const isVehicleTracking = searchParams.plateNumber && searchParams.plateNumber.trim() !== '';
    
    if (isVehicleTracking) {
      this.enableTrackingMode(searchParams.plateNumber.trim());
      return;
    } else {
  ...
  enableTrackingMode(licensePlate: string): void {
    ...
    this.loadTrackingData(licensePlate);
  }
    ...
    this.eventService.getListEvents(trackingQueryParams).subscribe({
      next: (response: any) => {
        ...
        if (events.length > 0) {
          ...
          this.trackingLocations = [];
          this.snackBar.open('Không tìm thấy dữ liệu tracking cho biển số này', 'Đóng', { duration: 3000 });
        }
